---
name: hallmark
description: "Anti-AI-slop design skill for web marketing, product UI, and native mobile; includes greenfield builds, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build or redesign an interface, invokes Hallmark by name, or uses audit/redesign/study."
version: 1.3.0
---
...
7. **Default-reflex interruption.** Before committing to a page shape, name the first generic solution an AI would likely choose for this brief and reject or justify it. Ask: *“Am I repeating an AI habit, or does this choice follow from the audience, domain, content, and conversion goal?”* Load [`references/default-reflex.md`](references/default-reflex.md), record the short reflex check in the preview, and revise whenever a choice exists only because it is familiar.

---

## Platform and product-scope dispatch

Run this dispatch before deciding component scope or entering the marketing-page Design flow. Hallmark supports three distinct media; never apply one medium's defaults to another.

### A. Native mobile

Route to native when project signals include SwiftUI/UIKit, Jetpack Compose/Android Views, React Native/Expo, or Flutter, or when the user explicitly requests an iOS/Android/native screen.

- Load [`references/mobile-native.md`](references/mobile-native.md) immediately.
- Preserve the platform's existing navigation, theme, component, and state architecture.
- Skip web macrostructure, marketing nav/footer, hero enrichment, CSS breakpoint, and browser-chrome rules.
- Use the universal intent of honest copy, hierarchy, restraint, accessibility, token discipline, and state completeness, then run the platform checklist and evidence contract in `mobile-native.md`.
- State the route: *"Platform: React Native · targets: iOS + Android. Native flow; web marketing macrostructure skipped."*

### B. Web product UI

Route to product UI when the target is an authenticated app, dashboard, admin/operations console, editor, inbox, analytics surface, settings area, CRM/ERP, or data-dense repeated workflow.

- Load [`references/product-ui.md`](references/product-ui.md) immediately.
- Load [`references/data-viz.md`](references/data-viz.md) only when charts or analytical graphics are present.
- Skip marketing hero, enrichment, testimonial/CTA sequence, marketing nav archetypes, and marketing footer rotation.
- Select a product job and shell (monitor, search/triage, operate, create/edit, configure, investigate), density mode, state model, permissions model, and responsive strategy.
- State the route: *"Scope: product UI · job: Operate · shell: filters + compact table + detail panel. Marketing page flow skipped."*

### C. Web marketing/content

Landing pages, public product pages, portfolios, editorial pages, campaigns, docs roots, and brand sites continue through the full Design flow below.

### Ambiguous projects

Use the requested route/file as the deciding signal. A SaaS repository can contain a public marketing route and an authenticated product route; classify the target, not the company. If one request genuinely spans both, split the work into two surfaces with one shared system — marketing flow for the public route, product flow for the app — and load [`references/design-system-enforcement.md`](references/design-system-enforcement.md) to keep them coherent.
...
5. **Framework / platform** — Next.js (`next` in deps), Astro (`astro`), Vue (`vue`), Svelte / SvelteKit (`svelte` / `@sveltejs/kit`), Remix (`@remix-run/*`), vanilla HTML, iOS (`*.xcodeproj`, SwiftUI/UIKit), Android (`build.gradle*`, Compose/Views), React Native / Expo, or Flutter (`pubspec.yaml`). Platform detection controls the dispatch above; never report a native project as merely “no web framework”.
...
- **`design.md` found** → emit *"`design.md` detected at project root — this is a system-managed project. Reading the locked design system; subsequent picks defer to it."* Then read the file in full, load [`references/design-system-enforcement.md`](references/design-system-enforcement.md), and use the file as the source of truth for genre / theme / typography / spacing / motion / CTA voice. Skip Step 1's catalog/custom dispatch; the system is already chosen. Proceed to a marketing macrostructure only for marketing scope; product/native routes use their own shell while preserving the locked brand axes.
...
### 1.5. Domain and conversion dispatch

For web marketing/content work, read [`references/category-taxonomy.md`](references/category-taxonomy.md) and classify the brief across independent axes: surface, business category, business model, decision mode, conversion, implementation platform, and art direction. This taxonomy is generative: a site is a combination such as `commerce × fashion × fit-led × Shopify × direct-commerce`, not a prebuilt “fashion shop” skill.

Then read the slim index at [`references/domain-dispatch.md`](references/domain-dispatch.md). Load a file from `references/domains/` only when a specialised pack matches; these packs add deeper rules and do **not** define the list of supported categories. Do not load every domain reference.

If the page has a clear acquisition goal, also load [`references/conversion-funnels.md`](references/conversion-funnels.md) and select exactly one primary conversion mode: lead generation, click-through, subscription, video-led, direct commerce, or discovery. A domain and a funnel are independent layers: `professional-services + lead-generation` and `local-services + click-through` should not produce the same page.

Load [`references/style-directions.md`](references/style-directions.md) when the user asks for a trend-led, expressive, unusual, colourful, immersive, or 3D direction. Select a direction because it clarifies product character or buyer behaviour, not because it is fashionable. If the direction uses spatial/3D media, also load [`references/spatial-3d.md`](references/spatial-3d.md) and pass its value, fallback, accessibility, and performance gates.

State the composition before choosing the macrostructure:

> *Surface: storefront · category: retail/electronics · model: focused DTC · decision: specification + compatibility · platform: Shopify · conversion: direct commerce · proof: specs + comparison + verified reviews.*

Then run [`references/default-reflex.md`](references/default-reflex.md) and state one line:

> *Default reflex: rejected centred gradient hero + three equal cards; chosen product-led catalogue with asymmetric merchandising because comparison drives this purchase.*

If no specialised pack fits, use the generic category profile in `category-taxonomy.md` and continue; do not label the brief unsupported and do not force it into a near match. Domain references constrain content, proof, information density, and mobile priorities; they do not override the selected genre, theme, platform architecture, or existing design system.
...
- [`category-taxonomy.md`](references/category-taxonomy.md) — load for every web marketing/content page; compose independent category axes rather than selecting a hard-coded site template.
- [`domain-dispatch.md`](references/domain-dispatch.md) — load after the taxonomy, then load only a matching specialised file in `references/domains/` when one exists. Skip for product UI and native.
- [`conversion-funnels.md`](references/conversion-funnels.md) — load when the marketing page has a clear acquisition or commerce goal; select one primary conversion mode.
- [`default-reflex.md`](references/default-reflex.md) — load before committing to structure on every new page or redesign; identify and interrupt likely AI-default choices.
- [`style-directions.md`](references/style-directions.md) — load when the brief asks for trend-led, expressive, colourful, immersive, unusual, or 3D art direction.
- [`spatial-3d.md`](references/spatial-3d.md) — load only when 3D, WebGL, spatial product viewing, or an interactive scene is actually proposed.
- [`product-ui.md`](references/product-ui.md) — load for authenticated applications, dashboards, admin/operations tools, editors, settings, and data-dense workflows. This replaces marketing-page structure for that target.
- [`mobile-native.md`](references/mobile-native.md) — load for iOS, Android, React Native/Expo, and Flutter. This replaces web-only layout and verification rules.
- [`data-viz.md`](references/data-viz.md) — load when charts, plots, maps, cohorts, funnels, telemetry, or analytical comparison are present.
- [`design-system-enforcement.md`](references/design-system-enforcement.md) — load whenever `design.md`, machine-readable tokens, a framework theme, or an established shared component library exists.
...
- **Default reflex** · rejected equal feature cards; proof-led split sequence instead
- **Slop test** · 58 / 58 ✓ (run after Build)
- **Diversification** · differs from Newsprint on display style + accent hue
```

**Seven required bullets, one optional, plus a CTA line:**
...
6. **Default reflex** — one likely AI-default choice rejected or explicitly justified, plus the content/domain reason for the replacement.
7. **Slop test** — `58 / 58 ✓` if all gates pass, or `N / 58 — fails: <gate numbers>` if any are open. Run the slop test BEFORE writing this row; the slop test is Step 7.
8. **Diversification** *(optional, only when `.hallmark/log.json` has prior entries)* — what axes differ vs the previous run.
...
**Scope overrides:** for product UI, follow the application-shell, density, state, permission, and output contract in [`product-ui.md`](references/product-ui.md); do not manufacture a hero/CTA/footer sequence. For native, follow [`mobile-native.md`](references/mobile-native.md), use platform semantics and verification, and do not emit CSS/tokens formats that the target framework cannot consume. When a design system exists, every scope also follows [`design-system-enforcement.md`](references/design-system-enforcement.md).
...
**Scope the evidence honestly.** The 58-gate file is the web-marketing suite. Product UI runs every applicable universal web gate plus the product-specific checklist; marketing-only hero/nav/footer/diversification gates are `N/A`, not silently counted as passes. Native runs the universal design intent plus the platform verification contract in `mobile-native.md`; CSS/HTML/browser gates are `N/A`. Report `pass / fail / N/A / not-run` counts by suite. Never write “58/58” for a product or native artifact whose medium makes some gates inapplicable.
