...
1. Tell the user to open a **separate, real terminal** (not this Codex
   session) and run:
   ```bash
   pnpm board
   ```
...
3. Explain what they'll see:
   ...
   - The board only ever writes `Status`/`Assignee`. Task content (title,
     files, acceptance criteria, skills, notes) stays owned by Codex /
     `scope-planner` — the board never edits it.
4. Tell them: once a task is sitting in **Ready**, run `/run-task` here in
   Codex to pick it up — it implements the task in an isolated
   ...
   needed.
5. The server runs **outside** the Codex session entirely — it's a
