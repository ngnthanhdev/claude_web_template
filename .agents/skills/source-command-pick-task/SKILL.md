---
name: "source-command-pick-task"
description: "Show the next unchecked task in the current layer, load its named skills, and restate its acceptance criteria."
---

# source-command-pick-task

Use this skill when the user asks to run the migrated source command `pick-task`.

## Command Template

1. Read `AGENTS.md`'s "Current Layer" pointer to find the active
...
4. Load each skill named in the task's **Skills** field now (read the
   relevant `.Codex/skills/<name>/SKILL.md`) so the context is ready before
