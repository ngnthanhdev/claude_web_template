...
> **Template fit (kitvera/`apps/web`).** A naming aid only. Once an effect is named, build it with **Framer Motion** via **`web-animations`**, gated by **`motion-design-principles`**, with a `prefers-reduced-motion` fallback.
