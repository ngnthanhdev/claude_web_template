...
> **Template fit (kitvera/`apps/web`).** Physical/fluid-motion reference. Build the motion with **Framer Motion** via **`web-animations`** (the template's animation lib — no second one); keep **`motion-design-principles`** as the gate and always ship a `prefers-reduced-motion` fallback.
