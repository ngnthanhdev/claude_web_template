...
> **Template fit (kitvera/`apps/web`).** Emil's craft depth for animation + component polish — the sharpest reference for "why premium UI feels premium." Implement through **`web-animations`** (Framer Motion) — never add a second animation library; **`motion-design-principles`** stays the WHETHER / how-much gate, and every effect ships a `prefers-reduced-motion` fallback. Style through `web-styling` tokens (`apps/web/src/app/globals.css`); all copy is next-intl (`vi`/`en`).
