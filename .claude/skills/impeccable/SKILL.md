...
> **Lean install (this template).** Only the design **playbooks** (`reference/*.md`) and the deterministic **anti-pattern detector** are vendored — not the full `npx impeccable` toolchain. The `context.mjs` / `init` / `document` / `doctor` / `hooks` / `pin` / `live` / `colorize`-palette / image-gen scripts are intentionally **not** copied, so those sub-commands are unavailable here; use this skill as an **audit → critique → harden → polish gate + design methodology**. Run the detector with:
> `node .claude/skills/impeccable/scripts/detect.mjs <file-or-dir>` — a static source scan (overused fonts, source-level gradients/patterns). Deeper **contrast / rendered-color** checks (e.g. the low-contrast bug this template already hit) need browser mode against a served page: `... detect.mjs --url http://localhost:3001/...`; run that in a real terminal, never auto-run in-session.
>
> **Template roles (kitvera).** The UI skills layer: **`taste-skill`** = aesthetic direction for landing/marketing surfaces; **`ui-ux-pro-max`** = product-UI reference data (palettes/fonts, a11y, forms, charts, per-stack); **`impeccable`** (this) = the audit/harden/polish **gate**, run last. None of them bypasses implementation via **`web-styling`** (Tailwind tokens + shadcn/ui), **`web-responsive`**, **`web-animations`** (Framer Motion). Respect: tokens in `apps/web/src/app/globals.css`, next-intl copy (`vi`/`en`, no hard-coded strings), CSP nonce + `force-dynamic`.
