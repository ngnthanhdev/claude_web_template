...
> **Template integration (this repo — `apps/web`).** Use this skill for **marketing / landing / home / redesign** surfaces (e.g. the `/[locale]` home and its hero/CTA/discovery sections). It is explicitly **not** for product UI — collection tables, filters, the account panel, forms, and multi-step flows stay with **`ui-ux-pro-max`** (which owns the curated palettes/fonts, a11y, forms, charts, and per-stack data). This skill sets **direction**; the actual code still goes through **`web-styling`** (Tailwind tokens + shadcn/ui), **`web-responsive`**, and **`web-animations`** (Framer Motion) — never hand-roll a token or add a second animation library. Read **`apps/web/DESIGN.md`** first — the design brief + token source of truth (raw tokens live in `apps/web/src/app/globals.css`). All copy is next-intl (no hard-coded strings, both `vi`/`en`), and pages render under a CSP nonce + `force-dynamic`. Run the **`impeccable`** audit as the final gate before calling a screen done.
