...
- **Current Layer:** Layer 7 — Commerce Wave 1: Core Purchase Path (**complete 2026-08-02**). All 11 tasks across 5 rounds are done, code+security reviewed, and merged; the review fix-forwards (idempotent-replay-before-rate-limit, duplicate-line rejection, cross-user account-cache clear, accumulate-not-replace pagers, `main.ts` download-token log redaction, the shared `downloadIssueRequestSchema` contract reconciliation) all landed. Unit/component suites green (shared 116, web 249, api 111) and the commerce/entitlements/composed-flow disposable-Postgres integration suites gate merges in `ci.yml` (CI green). The Playwright happy-path e2e is authored + static-valid; its full browser run is the out-of-session terminal step. Recorded in `tasks/done.md`. Payment production, the object-storage provider, and admin MFA remain go-live blockers.
- **Current Task:** none — the next layer is a user-directed choice, since three approved designs plus deferred commerce Wave 2 are all candidate inputs to separate future `/scope-breakdown` passes: commerce **Wave 2** (`docs/specs/2026-07-31-commerce-purchase-surface-design.md` — wishlist, coupons/referrals, discount-quotes, verified reviews), **seller authoring** (`docs/specs/2026-08-01-seller-authoring-design.md`), and **admin surface** (`docs/specs/2026-08-01-admin-surface-design.md`). Run `/scope-breakdown` on the chosen design to generate `tasks/layer-8-todo.md`.
