...
| ---------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`         | every pull request; push to `main`/`develop` | Quality gate: `pnpm install --frozen-lockfile` then `pnpm turbo run lint typecheck test` across every app/package in the workspace. Also runs a separate **non-blocking** `design-detector` job — the vendored `impeccable` static anti-pattern detector over `apps/web/src` (off-token colors/fonts vs `apps/web/DESIGN.md`); `continue-on-error` like the `security.yml` scanners until the design system is bespoke. |
