...
| ---------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`         | every pull request; push to `main`/`develop` | Quality gate: `pnpm install --frozen-lockfile` then `pnpm turbo run lint typecheck test` across every app/package in the workspace. Also runs a separate **non-blocking** `design-detector` job — the vendored `impeccable` static anti-pattern detector over `apps/web/src` (off-token colors/fonts vs `apps/web/DESIGN.md`); `continue-on-error` like `security.yml`'s Semgrep step until the design system is bespoke. |
| `security.yml`   | every pull request; push to `main`/`develop` | Gitleaks (secret scan), Semgrep (SAST against `p/typescript p/javascript p/owasp-top-ten p/nodejsscan`), and `pnpm audit --audit-level=high` (dependency vulnerabilities). Gitleaks and `pnpm audit` now **block merges** (the apps carry real source since Layers 1–5); Semgrep stays `continue-on-error` pending a tuning pass. See `docs/SECURITY.md` for the gate status and the dated dependency-audit exceptions.   |
